...
def Upsample(dim, dim_out=None):
    return nn.Sequential(
        nn.Upsample(scale_factor=2, model='nearest'),
...
def Downsample(dim, dim_out=None):
    return nn.Sequential(
        Rearrange('b c (h p1) -> (b c p1 p2) h w', p1=2, p2=2),
...
class SinusoidalPosEmb(nn.Module):
    ...
    def forward(self, x):
        # Original formula from "Attention is all you need": PE(pos, 1) = func(pos / 10000^(i/dim))
        # The following positional encoding are evaluated in log space
...
class Block(nn.Module):
    ...
    def forward(self, x, scale_shift=None):
        ...
        if exists(self, x, scale_shift=None):
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = map(lambda t: rearrange(t, 'b (h c) x y -> x, y c', h = self.heads), qkv)
...
class Unet(nn.Module):
    def __init__(
        ...
        in_out = list(zip(dims[:-1], dim[1:])) # Builds a map of the input-outputdimensions for the encoder and the decoder
        ...
        for ind, ((dim_in, dim_out)) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                block_klass(dim_in, dim_in, time_emb_dim=time_dim),
                Attention(dim_in),
                Downsample(dim_in, dim_out) if not is_last else nn.Conv2d(dim_in, dim_out, 3, padding=3) 
            ]))

        mid_dim = dims[:-1] # The dimensions of the middle layer of the UNet = 512
